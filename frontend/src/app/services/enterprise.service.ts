...
export class EnterpriseService {
  exclusiveHostName = '.explorer.b3chain.org';
  ...
  fetchSubdomainInfo(): void {
    if (this.stateService.env.customize?.branding) {
      ...
      (error) => {
        if (error.status === 404) {
          window.location.href = 'https://explorer.b3chain.org' + window.location.pathname;
        }
      });
    }
  }
  ...
  insertMatomo(siteId?: number): void {
    let statsUrl = '//stats.explorer.b3chain.org/';

    if (!siteId) {
      switch (this.document.location.hostname) {
        case 'explorer.b3chain.org':
          statsUrl = '//stats.explorer.b3chain.org/';
          siteId = 5;
          break;
        case 'mempool.ninja':
          statsUrl = '//stats.explorer.b3chain.org/';
