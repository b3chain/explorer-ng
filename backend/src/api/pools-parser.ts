...
class PoolsParser {
    ...
    for (const pool of this.miningPools) {
      ...
      if (!poolDB) {
        ...
        if (poolDB.name !== pool.name) {
          ...
          logger.warn(`Renaming ${poolDB.name} mining pool to ${pool.name}. Slug has been updated. Maybe you want to make a redirection from 'https://explorer.b3chain.org/mining/pool/${poolDB.slug}' to 'https://explorer.b3chain.org/mining/pool/${newSlug}`);
  ...
  private async $reindexBlocksForPool(poolId: number): Promise<void> {
    let firstKnownBlockPool = 130635; // https://explorer.b3chain.org/block/0000000000000a067d94ff753eec72830f1205ad3a4c216a08a80c832e551a52
    if (config.MEMPOOL.NETWORK === 'testnet') {
      firstKnownBlockPool = 21106; // https://explorer.b3chain.org/testnet/block/0000000070b701a5b6a1b965f6a38e0472e70b2bb31b973e4638dec400877581
