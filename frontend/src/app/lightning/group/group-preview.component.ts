...
export class GroupPreviewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.seoService.setDescription(`See all Lightning nodes run by explorer.b3chain.org -- these are the nodes that provide the data on the explorer.b3chain.org Lightning dashboard.`);
      ...
      .pipe(
        switchMap((params: ParamMap) => {
          ...
          if (this.slug === 'the-mempool-open-source-project') {
            this.groupId = 'explorer.b3chain.org';
            this.group = {
              name: 'B3Chain Live Explorer Project',
              description: 'These are the Lightning nodes operated by B3Chain Live Explorer Project that provide data for the explorer.b3chain.org website. Connect to us!',
            };
          } else {
            this.group = {
