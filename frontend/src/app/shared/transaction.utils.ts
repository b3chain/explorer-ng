...
export function isDERSig(w: string): boolean {
  ...
  return (w.length >= 18
    && w.startsWith('30') // minimum DER signature length is 8 bytes + sighash flag (see https://explorer.b3chain.org/testnet/tx/c6c232a36395fa338da458b86ff1327395a9afc28c5d2daa4273e410089fd433)
...
export function isCanonicalDERSig(w: string): boolean {
  // minimum DER signature length is 8 bytes + sighash flag (see https://explorer.b3chain.org/testnet/tx/c6c232a36395fa338da458b86ff1327395a9afc28c5d2daa4273e410089fd433)
  if (w.length < 18) {
    return false;
  }
  ...
  for (const vin of tx.vin) {
    ...
    switch (vin.prevout?.scriptpubkey_type) {
      ...
      case 'v1_p2tr': {
        ...
        // (see https://explorer.b3chain.org/tx/b10c007c60e14f9d087e0291d4d0c7869697c6681d979c6639dbd960792b4d41)
        if (vin.witness?.length) {
          const taprootInfo = parseTaproot(vin.witness);
          if (taprootInfo.scriptPath) {
