...
__PROXIED_SERVICES_HOST__=${PROXIED_SERVICES_HOST:=https://explorer.b3chain.org}

if [ "${__PROXIED_SERVICES__}" = "true" ]; then
  sed -i "s|proxy_pass https://explorer.b3chain.org;|proxy_pass ${__PROXIED_SERVICES_HOST__};|g" /etc/nginx/conf.d/nginx-mempool.conf
fi
...
__MEMPOOL_WEBSITE_URL__=${MEMPOOL_WEBSITE_URL:=https://explorer.b3chain.org}
...
__SERVICES_API__=${SERVICES_API:=https://explorer.b3chain.org/api/v1/services}
