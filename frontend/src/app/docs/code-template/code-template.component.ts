...
init();`;
  ...
  wrapCommonJS(code: any) {
    ...
    if (code.codeTemplate) {
      ...
      let importText = `<script src="https://explorer.b3chain.org/mempool.js"></script>`;
      if (this.env.BASE_MODULE === 'liquid') {
...
yarn add @mempool/liquid.js`;
  ...
  wrapPythonTemplate(code: any) {
    return ( ( this.network === 'testnet' || this.network === 'testnet4' || this.network === 'signet' ) ? ( code.codeTemplate.python.replace( 'wss://explorer.b3chain.org/api/v1/ws', 'wss://explorer.b3chain.org/' + this.network + '/api/v1/ws' ) ) : code.codeTemplate.python );
  }
