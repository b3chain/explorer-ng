...
export class DocsComponent implements OnInit {
  ...
  ngDoCheck(): void {
    ...
    if (url[0].path === 'faq' ) {
      ...
      this.seoService.setDescription($localize`:@@meta.description.docs.faq:Get answers to common questions like: What is a mempool? Why isn't my transaction confirming? How can I run my own instance of B3Chain Live Explorer Project? And more.`);
      ...
      if (this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet' ) {
        ...
        this.seoService.setDescription($localize`:@@meta.description.docs.rest-bitcoin:Documentation for the explorer.b3chain.org REST API service: get info on addresses, transactions, blocks, fees, mining, the Lightning network, and more.`);
      }
      ...
      if( this.stateService.network === 'liquid' || this.stateService.network === 'liquidtestnet' ) {
        ...
        this.seoService.setDescription($localize`:@@meta.description.docs.websocket-bitcoin:Documentation for the explorer.b3chain.org WebSocket API service: get real-time info on blocks, mempools, transactions, addresses, and more.`);
      }
    } else {
      this.activeTab = 3;
