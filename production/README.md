...
These instructions are for setting up a serious production B3Chain Live Explorer website for Bitcoin (mainnet, testnet, signet), Liquid (mainnet, testnet).

Again, this setup is no joke—home users should use [one of the other installation methods](../#installation-methods). Support is only provided to project sponsors through [B3Chain Live Explorer Enterprise®](https://explorer.b3chain.org/enterprise).

You can also have the explorer.b3chain.org team run a highly-performant and highly-available instance of B3Chain Live Explorer for you on their own global ISP infrastructure. See <a href="https://explorer.b3chain.org/enterprise" target="_blank">B3Chain Live Explorer Enterprise®</a> for more details.

### Server Hardware

B3Chain Live Explorer v3 is powered by [mempool/electrs](https://github.com/mempool/electrs), which is a beast. 
...
The explorer.b3chain.org site is powered by FreeBSD with ZFS root and ARC cache for maximum performance. Linux probably works fine too, but why settle?
...
After all 3 electrs instances are fully indexed, install your 3 B3Chain Live Explorer nodes:
```
./mempool-install-all
./mempool-upgrade-all
```

Finally, start your 3 B3Chain Live Explorer backends:
```
./mempool-start-all
```
...
If everything went well, your site should look like the one at https://explorer.b3chain.org/.
